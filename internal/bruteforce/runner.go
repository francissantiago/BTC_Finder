...
// StartWithCallback runs the worker pool with a callback when a match is found
// Useful for distributed mode where we need to notify external systems
func (r *Runner) StartWithCallback(ctx context.Context, onMatch func(privateKey string), onProgress func(keysChecked int64, lastChecksum string)) error {
	logging.Info("Runner starting with callback, %d workers", r.workers)
	...
	// If checkpoint exists, try to resume
	if _, err := os.Stat(r.checkpointFile); err == nil {
		b, err := os.ReadFile(r.checkpointFile)
		if err == nil {
			s := string(b)
	...
	for {
		...
		case <-checkpointTicker.C:
			...
			if onProgress != nil {
				r.mu.Lock()
				keysChecked := r.testedCount
				r.mu.Unlock()
				onProgress(keysChecked, chk)
			}
		}
	}
}
